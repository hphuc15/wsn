...
static const char *TAG = "[HW][IO][BUTTON]";

static void (*s_short_cb)(void) = NULL;
static void (*s_long_cb)(void)  = NULL;

static void btn_on_short_press(void){
    Utils_LogI(TAG, "Short press.");
    if (s_short_cb) {
        s_short_cb();
    }
}

static void btn_on_long_press(void){
    Utils_LogI(TAG, "Long press.");
    if (s_long_cb) {
        s_long_cb();
    }
}

static void short_press_task(void *arg){
    btn_on_short_press();
    vTaskDelete(NULL);
}
    ...
    while (1) {
        if (gpio_get_level(CFG_WSN_GATEWAY_GPIO_WAKE_BTN) == 0) {
            ...
            if (long_triggered) {
                ...
                xTaskCreate(short_press_task, "btn_short", 4096, NULL, 3, NULL);
            }
