...
static const char *TAG_SLEEP = "[HW][POWER][SLEEP]";
static const char *TAG_GPIO  = "[HW][POWER][GPIO]";
...
int power_sleep(void){
    ...
    if (rtc_gpio_init(POWER_GPIO_WAKE_BTN) != ESP_OK)                   { return -1; }
    if (rtc_gpio_pullup_en(POWER_GPIO_WAKE_BTN) != ESP_OK)              { return -1; }
    if (rtc_gpio_pulldown_dis(POWER_GPIO_WAKE_BTN) != ESP_OK)           { return -1; }
    if (esp_sleep_enable_ext0_wakeup(POWER_GPIO_WAKE_BTN, 0) != ESP_OK) { return -1; }
...
int power_led_blink(bool on){
    if (on) {
        if (s_led_blink_task_handle == NULL) {
            BaseType_t ret = xTaskCreatePinnedToCore(led_blink_task, "led_blink", 1024, NULL, 4, &s_led_blink_task_handle, 1);
            if (ret != pdPASS) {
                return -1;
            }
        }
...
int power_init(void){
    ...
    if (cause == ESP_SLEEP_WAKEUP_EXT0) {
        Utils_LogI(TAG_SLEEP, "Woke from deep sleep by button.");
    } else {
