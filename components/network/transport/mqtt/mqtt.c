...
static const char *TAG = "[NW][TRANSPORT][MQTT]";
...
int mqtt_init(const char *host, uint32_t port, const char *topic, const char *username, const char *password, bool tls){
    if (!host || !topic) {
        Utils_LogE(TAG, "Invalid host/topic.");
        return -1;
    }
    ...
    esp_mqtt_client_config_t cfg = {
        .broker = {
            .address = {
                .hostname  = host,
                .port      = port,
                .transport = tls ? MQTT_TRANSPORT_OVER_WSS : MQTT_TRANSPORT_OVER_WS,
                .path      = "/mqtt",
            },
    ...
    if (!s_client) {
        Utils_LogE(TAG, "Failed to init MQTT client.");
        return -1;
    }
    ...
    EventBits_t bits = xEventGroupWaitBits(s_mqtt_eg, MQTT_CONNECTED_BIT | MQTT_FAIL_BIT,
        pdFALSE, pdFALSE, pdMS_TO_TICKS(MQTT_CONNECT_TIMEOUT_MS));

    if (!(bits & MQTT_CONNECTED_BIT) || (bits & MQTT_FAIL_BIT)) {
        Utils_LogE(TAG, "Failed to connect to broker.");
        return -1;
    }
