...
/**
 * @brief Init MQTT client and connect to broker. Blocks up to 10s.
...
/** @brief Publish data to the configured topic at QoS 1. @return 0 on success, -1 on error. */
int mqtt_publish(const char *data);

/** @brief Stop and destroy the MQTT client. Safe to call if not initialized. */
