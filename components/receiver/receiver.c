...
static const char *TAG_LORA = "[RECEIVER][LORA]";
...
static void on_rx_done(SX127x_Dev *dev, const SX127x_Packet *packet){
    ...
    Utils_LogI(TAG_LORA, "RX: id=%u temp=%.2fC humi=%.2f%% soil=%.2f%% (rssi=%d snr=%.2f).",
        item.device_id, item.temp_c / 100.0f, item.humi_rh / 100.0f, item.soil_humi_vwc / 100.0f,
...
static int lora_init(void){
    ...
    Utils_LogI(TAG, "SX127x ready: freq=%lu bw=%d sf=%d cr=%d preamble=%u sync=0x%02X crc=%d implicit=%d txpow=%d",
...
int receiver_init(void){
    if (gpio_init() != 0) {
        Utils_LogE(TAG, "Failed to init GPIO.");
        return -1;
    }

    if (spi_init() != 0) {
        Utils_LogE(TAG, "Failed to init SPI.");
        return -1;
    }
    ...
    if (lora_init() != 0) {
        Utils_LogE(TAG, "Failed to init LoRa.");
        return -1;
    }
...
int receiver_start(void){
    if (s_rx_queue == NULL) {
        Utils_LogE(TAG, "Not initialized.");
        return -1;
    }
